...
public class MultiRowItem extends AbstractMultiItem {
	...
	public MultiRowItem(String prefix, String[] items, String query, ZServer zs) {
	...
	public ZabbixItem[] getItemData(Connection con, String hostname, int timeout) throws SQLException {		
		...
		for (String item: items)
			values.put(item, new ZabbixItem(hostname, name+item, noData,clock, this));
		...
		while (rs.next()) {
			...
			if (fetchedVal != null && values.containsKey(fetchedName)) {
				...
				values.put(fetchedName, new ZabbixItem(hostname, name+fetchedName, fetchedVal,clock,this));
